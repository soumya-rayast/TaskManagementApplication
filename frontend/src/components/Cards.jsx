...
const Cards = () => {
    ...
    return (
        <div className='grid grid-cols-4 gap-4 p-4 '>
            {data.map((item, index) => (
                <div className='flex flex-col justify-between border bg-gray-600 rounded-sm p-4'>
                    ...
                    <div className='mt-4 w-full flex items-center'>
                        <button className={`${item.status === 'Incomplete' ? "bg-red-400" : "bg-green-700" } bg-red-400 p-2 rounded w-3/6`}>{item.status}</button>
            ...
            <div className='flex flex-col justify-center border bg-gray-600 rounded-sm p-4 items-center hover:scale-105 hover:cursor-pointer transition-all duration-300'>
